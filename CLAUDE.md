...
- **Home (hub)**: header = avatar (esq, → Perfil) + "Bom treino, {nome}" + logo embaixo · foguinho à direita (→ StreakSheet que parabeniza). **Próximo treino** em destaque (calculado pela sequência A→B→C: o seguinte ao último concluído; continua de onde parou) + "Último: … há N dias" sutil. "Seu programa" lista os treinos → tocar abre **TreinoResumoSheet** (resumo + Iniciar). Metodologia NÃO é treino-por-dia-do-calendário. (Removido: "treino de hoje" e o strip "esta semana".)
